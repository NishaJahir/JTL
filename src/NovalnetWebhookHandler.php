...
class NovalnetWebhookHandler
{
    ...
    public function getOrderReference(): object
    {
        ...
        $novalnetOrder = Shop::Container()->getDB()->query('SELECT cNnorderid, cZahlungsmethode, cStatuswert, nBetrag FROM xplugin_novalnet_transaction_details WHERE nNntid = "' . $this->parentTid . '"',  ReturnType::SINGLE_OBJECT);
    ...
    {  
        ...
        if ($this->eventData['transaction']['update_type'] == 'STATUS') {
            ...
            $dueDateUpdateMessage = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_duedate_update_message', $this->languageCode), Preise::getLocalizedPriceWithoutFactor( $amount / 100, Frontend::getCurrency(), false ), $this->eventData['transaction']['due_date']);
            ...
            $amountUpdateMessage = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_amount_update_message', $this->languageCode), Preise::getLocalizedPriceWithoutFactor( $amount / 100, Frontend::getCurrency(), false ), date('d.m.Y'), date('H:i:s'));
        ...
        if (in_array($this->eventData['transaction']['payment_type'], ['INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'MULTIBANCO_CREDIT'])) {
            ...
            $this->eventData['order_paid_amount'] = $orderPaidAmount;
                ...
                if ($orderPaidAmount >= $orderAmount) {
                    ...
                    $orderStatus = $this->eventData['transaction']['payment_type'] == 'ONLINE_TRANSFER_CREDIT' ? '' : \BESTELLUNG_STATUS_BEZAHLT;
    ...
    */
     ...
     * @return none
     */
    public function sendMailNotification(array $webhookMessage): void
    {
