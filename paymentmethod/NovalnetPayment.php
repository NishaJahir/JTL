...
{     
    ...
    {       
        if(!empty($post['nn_seamless_payment_form_response'])) {
            $_SESSION['novalnet']['seamless_payment_form_response'] = json_decode(utf8_encode(base64_decode($post['nn_seamless_payment_form_response'])), true);
        }
        return true;
    }
    ...
    public function preparePaymentProcess(Bestellung $order): void
    {
        ...
        if($_SESSION['novalnet']['seamless_payment_form_response']['result']['status'] != 'SUCCESS') {
            ...
            if(!empty($_SESSION['novalnet']['seamless_payment_form_response']['booking_details']['do_redirect']) || $this->novalnetPaymentGateway->isRedirectPayment($paymentRequestData['transaction']['payment_type'])) {
    ...
    {           
        ...
        if($_SESSION['novalnet']['payment_response']['transaction']['status'] == 'CONFIRMED') {
        ...
        if(in_array($_SESSION['novalnet']['payment_key'], ['invoice', 'prepayment', 'instalment_invoice', 'guaranteed_invoice'])) {
            // Collecting the bank details required for the Invoice payment methods
            $transactionDetails .= $this->novalnetPaymentGateway->getBankdetailsInformation($order, $_SESSION['novalnet']['payment_response']);       
        }
